...
def create_error_page(conn, err_string):
    ...
    conn.send('<body><h1>Error</h1><hr/><p>{err_string}</p></body></html>'.encode())
    conn.close()
...
def handle_request(conn):
    ...
    if 'amount' in values:
        try:
            ...
            create_error_page(conn, "{values['amount']} is not a float")
            return
    ...
    if 'amount' in values:
        conn.send('<p>Transfer = %5.2f</p>\r\n'.encode() %(amount))
    conn.send('<p>New balance = %5.2f</p>\r\n'.encode('ascii') %(balance))
